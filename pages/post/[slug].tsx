    ...
    <main>
      ...
      {submitted ? (
        <div className="my-10 mx-auto flex max-w-2xl flex-col bg-green-500 p-10 text-white">
          <h3 className="text-3xl font-bold">Comment submitted</h3>
          <p>It will appear when it approves</p>
        </div>
      ) : (
        <form
        ...
        >
          ...
          <input
            type="submit"
            className="cursor-pointer rounded bg-green-700 py-2 px-4 font-bold text-white shadow hover:bg-green-600 focus:outline-none"
          />
        </form>
      )}
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  const query = `*[_type == "post" && slug.current == 'another-lorem'][0]{
    _id,
    _createdAt,
    title,
    author->{
